...
\begin{document}
	...
	\section{Functional Data Analysis}
		...
		\subsection{Hilbert Space of Square Integrable Functions}
			...
			\begin{definition} Square Integrable Functions
				...
				\end{equation}
				The set of all square integrable functions on $A$ is denoted by $\mathbb{L}^2(A)$.
			\end{definition}
	...
	\section{Test by Bugni and Horowitz (2021) - Two Samples}
		...
		\begin{equation}
			\begin{split}
				F_X(z) &= \mathbb{P}\left[X(t) \leq z(t) \quad \forall t \in \mathcal{I}\right] \quad z \in \mathbb{L}^2(\mathcal{I})\\
				F_Y(z) &= \mathbb{P}\left[Y(t) \leq z(t) \quad \forall t \in \mathcal{I}\right] \quad z \in \mathbb{L}^2(\mathcal{I})
			\end{split}
		\end{equation}
		...
		\subsection{Nullhypothesis}
			\begin{equation}
				\begin{split}
					H_0: \quad &F_X(z) = F_Y(z) \quad \forall z \in \mathbb{L}^2(\mathcal{I}) \\
					H_1: \quad &\mathbb{P}_{\mu}\left[F_X(Z) \neq F_Y(Z)\right] > 0
				\end{split}
			\end{equation}
			Here, $\mu$ is a probability measure on $\mathbb{L}^2(\mathcal{I})$ and $Z$ is a random function with probability distribution $\mu$. {\color{red} Doesn't this leave out the case where the Probability functions only differ on a set of $\mu$-measure zero?}
