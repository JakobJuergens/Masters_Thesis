...
# find combinations that add to 0.05
# combs <- expand_grid(tau_levels, nu_levels) %>%
#   filter(tau_levels + nu_levels == 0.05)
# comb_index <- tibble(
#   tau_index = match(combs$tau_levels, tau_levels),
#   nu_index = match(combs$nu_levels, nu_levels) + length(tau_levels))
...
sim_2_path <- paste0(sim_folders, "Simulation_2/")
