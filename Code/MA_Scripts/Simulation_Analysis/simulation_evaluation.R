...
sim_1_path <- paste0(sim_folders, "Simulation_1/12_06_2022/")
sim_2_path <- paste0(sim_folders, "Simulation_2/13_06_2022/")
sim_3_path <- paste0(sim_folders, "Simulation_3/13_06_2022/")
sim_4_path <- paste0(sim_folders, "Simulation_4/14_06_2022/")
# set Output path
...
sim_3_files <- purrr::map(
  .x = list.files(path = sim_3_path),
...
sim_3_val_test_func <- cbind(
  ...
  as_tibble(
    matrix(
      ...
      ), nrow = length(sim_3_files), ncol = length(tau_levels), byrow = TRUE
    )
  ),
  # nu test decisions
  as_tibble(
    matrix(
      ...
      ), nrow = length(sim_3_files), ncol = length(nu_levels), byrow = TRUE
    )
  )
)
colnames(sim_3_val_test_func) <- paste0(
  "test_func_",
  c(paste0("alpha_tau=", tau_levels), paste0("alpha_nu=", nu_levels))
)
...
sim_3_test_combined <- sim_3_val_test_func %>%
  mutate(comb_40_10 = test_combine(`test_func_alpha_tau=0.04`, `test_func_alpha_nu=0.01`),
         comb_30_20 = test_combine(`test_func_alpha_tau=0.03`, `test_func_alpha_nu=0.02`),
         comb_25_25 = test_combine(`test_func_alpha_tau=0.025`, `test_func_alpha_nu=0.025`),
         comb_20_30 = test_combine(`test_func_alpha_tau=0.02`, `test_func_alpha_nu=0.03`),
