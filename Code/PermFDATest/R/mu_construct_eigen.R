...
fourier_basis_coef_means_eigen <- function(w_func, eigen_func_obj, n_basis, domain = c(0, 1)) {
  # extract basis functions as function objects
  basis_functions <- purrr::map(
    .x = 1:n_basis,
    .f = function(i) {
      basis_func <- function(x) {
        fda::eval.fd(evalarg = x, fdobj = eigen_func_obj)[1, i]
      }
    }
  )
...
fourier_coef_mean_eigen <- function(w_func, basis_func, domain = c(0, 1)) {
