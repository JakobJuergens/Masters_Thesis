...
test_that("The generation of random fourier coefficients works", {
  ...
  rand_coef <- fourier_coef_sample(
    w_func = w_func, basis = fourier_basis, n_basis = n_basis,
...
test_that("The generation of random functions works (with a basis generated by fda)", {
  # create the weight function
  w_func <- function(x) {
    return(1)
  }
  ...
  n_basis <- 15
  # create the fourier basis
  fourier_basis <- fda::create.fourier.basis(rangeval = c(0, 1), nbasis = n_basis)
  # create vector of rho's
  rho <- seq(from = 2, to = 0.25, length.out = n_basis)
  # draw random function
  rand_func <- function_sample(
    w_func = w_func, basis = fourier_basis, n_basis = n_basis,
    rho = rho, u_sample_func = u_unif, domain = c(0, 1)
  )
