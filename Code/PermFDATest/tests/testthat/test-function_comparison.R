test_that("Zero calculation works via fourier coefficients with basis of period 2*pi", {
  fourier_basis <- fda::create.fourier.basis(
    rangeval = c(0, 2 * pi), nbasis = 15,
    period = 2 * pi
  )

  func_a <- fda::fd(
    coef = c(1, 1, 1, rep(0, times = 11), 0.5),
...
test_that("Function Comparison works via fourier coefficients with basis of period 2*pi", {
  ...
  func_a <- fda::fd(
    coef = c(4, 1, 1, rep(0, times = 11), 0.5),
    basisobj = fourier_basis
  )
...
test_that(paste0(
  ...
  fourier_basis <- fda::create.fourier.basis(
    rangeval = c(0, 2 * pi), nbasis = 5,
    period = 2 * pi
  )

  func_a <- fda::fd(
    coef = c(0, 0, 0, 0, 1 * sqrt(pi)),
    basisobj = fourier_basis
  )

  func_b <- fda::fd(
    coef = c(2 * sqrt(2 * pi), 0, 0, 0, -1 * sqrt(pi)),
    basisobj = fourier_basis
  )
...
test_that(paste0(
  ...
  fourier_basis <- fda::create.fourier.basis(
    rangeval = c(0, 1), nbasis = 5,
    period = 1
  )

  func_a <- fda::fd(
    coef = c(0, 0, 0, 0, 1 / sqrt(2)),
    basisobj = fourier_basis
  )

  func_b <- fda::fd(
    coef = c(2, 0, 0, 0, -1 / sqrt(2)),
    basisobj = fourier_basis
  )
