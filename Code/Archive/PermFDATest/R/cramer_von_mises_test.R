...
cramer_von_mises_tstat_fourier <- function(sample1, sample2, domain = c(0, 1), basis,
  ...
  for (i in 1:n_func) {
    ...
    cur_func <- coef_means + errors
    # calculate value of the empirical distribution functions
    emp_dist_vals_s1[i] <- empirical_dist_func_fourier(sample = sample1, func = cur_func, domain = domain)
    emp_dist_vals_s2[i] <- empirical_dist_func_fourier(sample = sample2, func = cur_func, domain = domain)
  }
  # calculate test statistic tau_hat
  tau_hat <- (ncol(sample1) + ncol(sample2)) * mean(x = (emp_dist_vals_s1 - emp_dist_vals_s2)^2, na.rm = FALSE)
  # return tau_hat
  return(tau_hat)
}
...
cramer_von_mises_tstat_eigen <- function(sample1, sample2, domain = c(0, 1), eigen_func_obj,
                                         w_func, rho, u_sample_func, n_func, ...) {
  stop('The Eigenfunction version of the Cramer-von Mises test is currently not implemented.')
}

#' This function calculates the realization of the cramer-von mises type test
#' on two given samples (for grid based variant)
#'
#' @param sample1: first sample, specified as a matrix of fourier coefficients
#' @param sample2: second sample, specified as a matrix of fourier coefficients
#' @param domain: Domain of the functional observations
#' @param basis: basis generated by fda
#' @param w_func: weight function for construction of measure
#' @param rho: a vector of length n_basis from a square summable sequence that is used
...
cramer_von_mises_tstat_grid <- function(sample1, sample2, domain = c(0, 1), basis,
  ...
  funcs_vals <- fourier_eval(x = grid, coefs = coefs_funcs, domain = domain)
  s1_vals <- fourier_eval(x = grid, coefs = sample1, domain = domain)
  s2_vals <- fourier_eval(x = grid, coefs = sample2, domain = domain)
